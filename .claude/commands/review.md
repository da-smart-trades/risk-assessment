...
```bash
uv run pytest tests/ --cov=src/cert_ra/api/domain/{feature} --cov-report=term-missing
```
...
mcp__pal__analyze(
    ...
    relevant_files=["src/cert_ra/api/domain/{feature}/..."]
)
```
...
diff <(grep -A20 "class.*Controller" src/cert_ra/api/domain/accounts/controllers.py) \
     <(grep -A20 "class.*Controller" src/cert_ra/api/domain/{feature}/controllers.py)
```
...
## Reference Files
- `src/cert_ra/api/domain/{feature}/...`
...
Quality Gates:
- ✓ make test passes
- ✓ make lint passes (pre-commit: ruff, ruff-fmt, mypy, codespell, biome + pyright + slotscheck)
- ✓ make biome / make lint-js passes
- ✓ make coverage (90%+)
