...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, WebSearch, mcp__deepwiki__read_wiki_structure, mcp__deepwiki__read_wiki_contents, mcp__deepwiki__ask_question, mcp__sequential-thinking__sequentialthinking, mcp__pal__planner
---
...
- `src/cert_ra/api/domain/` for backend patterns
- `resources/pages/` for frontend patterns
- `src/cert_ra/db/models/` for data model patterns
...
grep -r "class.*Controller" src/cert_ra/api/domain/ | head -5
grep -r "class.*Service" src/cert_ra/api/domain/ | head -5
...
1. `src/cert_ra/api/domain/{feature}/controllers.py` - Description
2. `src/cert_ra/api/domain/{feature}/services.py` - Description
...
3. **DeepWiki**: Library docs for Litestar, React, Inertia, SQLAlchemy, Temporal
4. **WebSearch**: Best practices, recent updates

**DeepWiki lookups for this project:**
...
Available repos: `litestar-org/litestar`, `litestar-org/litestar-vite`, `litestar-org/advanced-alchemy`, `sqlalchemy/sqlalchemy`, `facebook/react`, `inertiajs/inertia`, `shadcn-ui/ui`, `temporalio/sdk-python`.
...
- Controller: `src/cert_ra/api/domain/{feature}/controllers.py`
- Service: `src/cert_ra/api/domain/{feature}/services.py`
- Repository: `src/cert_ra/api/domain/{feature}/repositories.py`
- Models: `src/cert_ra/db/models/{model}.py`
...
```bash
git status --porcelain src/ resources/ | grep -v "^??"
```
