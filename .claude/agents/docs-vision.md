...
Implementation:
  ...
  - make lint (prek hooks: ruff, ruff-fmt, mypy, codespell, biome + pyright + slotscheck)
  - make biome / make lint-js (frontend linting only, if needed in isolation)
...
make test
make lint        # Python + JS (biome runs via prek)
make coverage
```
...
- `src/cert_ra/api/domain/{feature}/...` - Where this is used
