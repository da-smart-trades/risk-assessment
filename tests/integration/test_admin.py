...
async def test_admin_users_detail(
    ...
    response = await client.get(
        f"/admin/users/{user_id}/", headers=superuser_inertia_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert "props" in data
...
async def test_admin_users_create_page(
    ...
    response = await client.get(
        "/admin/users/create/", headers=superuser_inertia_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert "props" in data
...
async def test_admin_teams_detail(
    ...
    response = await client.get(
        f"/admin/teams/{team_id}/", headers=superuser_inertia_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert "props" in data
