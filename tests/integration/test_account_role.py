...
async def test_superuser_role_access(
    ...
    assert (
        ...
        == "Successfully assigned the 'superuser' role to user@example.com."
    )
    ...
    # User can now see all teams (3 seeded + 1 operator team auto-provisioned on startup)
    response = await client.get("/teams", headers=user_inertia_headers)
    assert response.status_code == 200
    data = response.json()
    # TeamListPage returns 'teams' list inside content
    assert len(data["props"]["content"]["teams"]) == 4
    ...
    assert len(superuser_data["props"]["content"]["teams"]) == 4
