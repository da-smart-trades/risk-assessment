...
from cert_ra.api.domain.accounts.services import UserOAuthAccountService
...
async def test_unlink_unknown_provider(
...
async def test_link_unknown_provider(
    client: AsyncClient, user_inertia_headers: dict[str, str]
) -> None:
...
async def test_can_unlink_with_password(session: AsyncSession) -> None:
    ...
    user = User(
        ...
        hashed_password="hashed_password_here",  # noqa: S106
    )
...
@pytest.mark.xfail(
    reason="OIDC SSO design (PR-1): uq_oauth_user_singleton enforces one OIDC "
    "provider per user; the multi-oauth scenario this test sets up is no longer "
    "reachable. PR-2 (OIDC core) replaces this test with the new single-IDP "
