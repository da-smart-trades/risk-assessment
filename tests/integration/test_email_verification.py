...
async def test_verify_email_invalid_token(client: AsyncClient) -> None:
    ...
    response = await client.get(
        "/verify-email/?token=invalid-token", follow_redirects=True
    )
    assert response.status_code == 200
...
async def test_verify_email_valid_token(
    ...
    async with sessionmaker() as session:
        ...
        await users_service.update(
            {"is_verified": False}, item_id=user.id, auto_commit=True
        )
    ...
    # Verify the email
    response = await client.get(
        f"/verify-email/?token={plain_token}", follow_redirects=True
    )
    assert response.status_code == 200
    assert "/login" in str(response.url)
