...
from cert_ra.api import config
from cert_ra.api.domain.accounts.services import RoleService, UserService
...
if TYPE_CHECKING:
    ...
    from cert_ra.db.models import Team, User
...
async def _seed_db(
    ...
    fixtures_path = db_settings.fixture_path
    metadata = UUIDAuditBase.registry.metadata
    # Enums declared with ``create_type=False`` on the model side are normally
    # materialized by Alembic migrations; the test harness builds the schema
    # via ``metadata.create_all`` (no migrations), so we have to create those
    # types here before the column DDL references them.
    ...
    async with engine.begin() as conn:
        ...
        for enum_cls, type_name in (
            ...
            (AlertTargetKind, "alerttargetkind"),
        ):
            values = ", ".join(f"'{m.value}'" for m in enum_cls)
            await conn.exec_driver_sql(f"DROP TYPE IF EXISTS {type_name} CASCADE")
            await conn.exec_driver_sql(f"CREATE TYPE {type_name} AS ENUM ({values})")
...
async def fx_client(app: Litestar) -> AsyncIterator[AsyncClient]:
    ...
    manager.app = _fresh_state_lifespan_middleware(app, manager._state)  # type: ignore[assignment]  # noqa: SLF001
...
async def fx_superuser_inertia_headers(client: AsyncClient) -> dict[str, str]:
    ...
    return await _get_auth_headers(
        client, "superuser@example.com", "Test_Password1!", inertia=True
    )
...
async def fx_user_inertia_headers(client: AsyncClient) -> dict[str, str]:
    ...
    return await _get_auth_headers(
        client, "user@example.com", "Test_Password2!", inertia=True
    )
