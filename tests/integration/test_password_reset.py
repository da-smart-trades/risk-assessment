...
from cert_ra.api.domain.accounts.services import EmailTokenService, UserService
...
async def test_forgot_password_submit_existing_email(client: AsyncClient) -> None:
    ...
    response = await client.post(
        "/forgot-password/", json={"email": "user@example.com"}, headers=headers
    )
...
async def test_forgot_password_submit_nonexistent_email(client: AsyncClient) -> None:
    ...
    response = await client.post(
        "/forgot-password/", json={"email": "nonexistent@example.com"}, headers=headers
    )
...
async def test_reset_password_page_with_valid_token(
    ...
    response = await client.get(
        "/reset-password/", params={"token": plain_token, "email": "user@example.com"}
    )
...
async def test_reset_password_complete_flow(
    ...
    new_password = "NewSecurePassword123!"  # noqa: S105
    response = await client.post(
        "/reset-password/",
...
async def test_email_token_create_and_validate(
    ...
    async with sessionmaker() as session:
        ...
        validated = await service.validate_token(
            plain_token=plain_token, token_type=TokenType.PASSWORD_RESET
        )
...
async def test_email_token_validate_wrong_type(
    ...
    async with sessionmaker() as session:
        ...
        validated = await service.validate_token(
            plain_token=plain_token, token_type=TokenType.EMAIL_VERIFICATION
        )
...
async def test_email_token_invalidate_existing(
    ...
    async with sessionmaker() as session:
        ...
        count = await service.invalidate_existing_tokens(
            email="user@example.com", token_type=TokenType.PASSWORD_RESET
        )
        assert count == 2
