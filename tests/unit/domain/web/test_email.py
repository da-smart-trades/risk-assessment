...
from cert_ra.api.domain.web.email import (
    EmailMessageService,
    TemplateRenderer,
...
def test_template_caching(temp_template_dir: Path) -> None:
    ...
    assert "test-template" in renderer._cache  # noqa: SLF001
...
def test_clear_cache(temp_template_dir: Path) -> None:
    ...
    assert len(renderer._cache) == 0  # noqa: SLF001
...
def mock_app_settings() -> MagicMock:
    ...
    settings = MagicMock()
    settings.name = "TestApp"
    settings.url = "http://test.example.com"
    return settings


@pytest.fixture
def mock_email_settings() -> MagicMock:
    """Create mock email settings for testing."""
    settings = MagicMock()
    settings.enabled = True
    settings.backend = "memory"
    settings.from_email = "test@example.com"
    settings.verification_token_expires_hours = 24
    settings.password_reset_token_expires_minutes = 60
    settings.invitation_token_expires_days = 7
    return settings


async def test_send_email_basic(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
    """Test basic email sending."""
    with (
        patch(
            "cert_ra.api.domain.web.email.get_app_settings",
            return_value=mock_app_settings,
        ),
        patch(
            "cert_ra.api.domain.web.email.get_email_settings",
...
async def test_send_email_when_disabled(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
    """Test that email is not sent when disabled."""
    mock_email_settings.enabled = False
...
async def test_send_email_list_recipients(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
...
async def test_send_email_html_to_text_conversion(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
...
async def test_send_template_email_fallback(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
...
def test_html_to_text_basic(
    ...
    with (
        ...
        service = EmailMessageService(mailer=mock_mailer)
        result = service._html_to_text("<p>Hello <b>World</b></p>")  # noqa: SLF001
...
def test_html_to_text_entities(
    ...
    with (
        ...
        service = EmailMessageService(mailer=mock_mailer)
        result = service._html_to_text("&nbsp;&amp;&lt;&gt;&quot;")  # noqa: SLF001
        assert "&" in result
        assert "<" in result
        assert ">" in result
...
def test_html_to_text_whitespace(
    ...
    with (
        ...
        service = EmailMessageService(mailer=mock_mailer)
        result = service._html_to_text("<p>  Hello   World  </p>")  # noqa: SLF001
        assert result == "Hello World"
...
async def test_send_email_exception_with_fail_silently(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
...
async def test_send_email_exception_without_fail_silently(
    mock_mailer: AsyncMock, mock_app_settings: MagicMock, mock_email_settings: MagicMock
) -> None:
