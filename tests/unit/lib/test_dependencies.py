...
from cert_ra.api.domain.accounts.dependencies import provide_user
...
async def test_filter_dependencies_with_litestar() -> None:
    ...
    async def test_route(
        ...
        assert (
            "CollectionFilter" in filter_types
            or len([f for f in filters if isinstance(f, CollectionFilter)]) >= 0
        )
