...
export default function ConnectedAccountsForm() {
	const { auth, githubOAuthEnabled, googleOAuthEnabled } = usePage<{
		auth: { user: User; enforcedProvider?: string | null }
	...
	// Per-team IDP enforcement: surface a proactive switch when the user's
	// team requires a provider they aren't yet signed in with.
	const enforcedProvider = auth.enforcedProvider ?? null
	const needsSwitch = !!enforcedProvider && !oauthAccounts.some((a) => a.oauthName === enforcedProvider)
	const enforcedLabel = enforcedProvider ? (PROVIDER_LABELS[enforcedProvider] ?? enforcedProvider) : ""

	const handleSwitch = () => {
		if (!enforcedProvider) return
		router.post(route("auth.switch-provider", { provider: enforcedProvider }))
	}
				...
				{needsSwitch && (
					<Alert>
						<AlertDescription className="flex items-center justify-between gap-4">
							<span>Your team requires {enforcedLabel} sign-in. Switch now to keep access.</span>
							<Button size="sm" onClick={handleSwitch}>
								Switch to {enforcedLabel}
							</Button>
						</AlertDescription>
					</Alert>
				)}
