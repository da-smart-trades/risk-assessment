...
export default function TeamMemberManager({ team, members, permissions }: Props) {
	...
	const changeRole = async (member: TeamPageMember, newRole: string) => {
		if (member.role === newRole) return
		setUpdatingRoleFor(member.id)
		try {
			await axios.post(route("teams:change-member-role", { team_slug: team.slug }), {
				userName: member.email,
				role: newRole,
			})
			toast({ description: `Updated ${member.name || member.email} to ${newRole}.`, variant: "success" })
			router.reload()
		} catch (error) {
			const detail = axios.isAxiosError(error) ? (error.response?.data?.detail as string | undefined) : undefined
			toast({ description: detail ?? "Failed to change role. Please try again.", variant: "destructive" })
		} finally {
							...
							<DialogContent>
								<form onSubmit={addMember}>
									<DialogHeader>
										...
										<DialogDescription>
											Enter an email address. Existing users are added immediately; new users receive an invitation to finish setting up their account.
										</DialogDescription>
									</DialogHeader>
									<div className="my-4">
								...
								<div className="flex items-center gap-4">
									...
									<div>
										<div className="flex items-center gap-2">
											...
											{member.role === "owner" || !permissions.canAddTeamMembers ? (
												<Badge className={cn("capitalize", roleStyles[member.role])}>
													{RoleIcon && <RoleIcon className="mr-1 h-3 w-3" />}
													{member.role}
												</Badge>
											) : (
												<Select value={member.role} onValueChange={(v) => changeRole(member, v)} disabled={updatingRoleFor === member.id}>
