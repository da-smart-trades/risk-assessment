...
import { ArrowLeft, CheckCircle2, Clock, Mail, Send, Trash2, UserPlus } from "lucide-react"
...
export default function TeamInvitations({ team, invitations, acceptedInvitations, permissions }: Props) {
	const pendingInvitations = invitations as PendingInvitation[]
	const accepted = acceptedInvitations ?? []
				...
				<div className="max-w-3xl space-y-6">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Invitations that have already been accepted.</CardDescription>
						</CardHeader>
						<CardContent>
							{accepted.length === 0 ? (
								<div className="flex flex-col items-center justify-center py-8 text-center">
									<CheckCircle2 className="h-12 w-12 text-muted-foreground" />
									<h3 className="mt-4 font-semibold text-lg">No accepted invitations yet</h3>
									<p className="mt-2 text-muted-foreground text-sm">When invitees accept, they'll appear here.</p>
								</div>
							) : (
								<div className="space-y-4">
										...
										<div key={invitation.id} className="flex items-center justify-between rounded-lg border p-4">
											<div className="flex items-center gap-4">
												...
												<div>
													...
													<div className="flex items-center gap-2 text-muted-foreground text-sm">
														<Clock className="h-3 w-3" />
														<span>{invitation.acceptedAt ? `Accepted ${formatDistanceToNow(new Date(invitation.acceptedAt), { addSuffix: true })}` : "Accepted"}</span>
														<span>·</span>
														<span>invited by {invitation.invitedByEmail}</span>
													</div>
												</div>
											</div>
										</div>
