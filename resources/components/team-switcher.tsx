			...
			<Popover open={open} onOpenChange={setOpen}>
						...
						{currentTeam === undefined ? (
							...
							<>
								<Avatar className="mr-2 h-5 w-5">
									<AvatarFallback>{getInitials(currentTeam.teamName || "Team")}</AvatarFallback>
								</Avatar>
				...
				<PopoverContent className="w-[200px] p-0 ">
					<Command className="rounded-lg border shadow-md ">
						<CommandInput className="border-none focus:ring-transparent" placeholder="Search team..." />
							...
							<CommandGroup key="teams" heading="Teams">
								{auth?.user?.teams.map((team) => (
									...
									>
										...
										{team.teamName}
										<Check className={cn("w- ml-auto h-4", currentTeam?.teamId === team.teamId ? "border-lime-700 stroke-lime-500 opacity-100" : "opacity-0")} />
									</CommandItem>
								))}
							</CommandGroup>
							<CommandSeparator />
					...
					<div>
						<div className="space-y-4 py-2 pb-4">
							...
							<div className="space-y-2">
								...
								<Input id="description" value={data.description} placeholder="Tell us a little about this team" onChange={(e) => setData("description", e.target.value)} />
