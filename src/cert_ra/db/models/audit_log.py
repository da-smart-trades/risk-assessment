...
class AuditAction(StrEnum):
    ...
    TEAM_ENFORCEMENT_REMINDER = "team.enforcement_reminder"

    # Market config actions (operator-only)
    MARKET_CONFIG_CREATED = "market_config.created"
    MARKET_CONFIG_UPDATED = "market_config.updated"
    MARKET_CONFIG_DELETED = "market_config.deleted"

    # Weighting profile actions (team admin/editor for team-scoped;
    # operator superuser for the global default)
    WEIGHTING_PROFILE_CREATED = "weighting_profile.created"
    WEIGHTING_PROFILE_UPDATED = "weighting_profile.updated"
    WEIGHTING_PROFILE_DELETED = "weighting_profile.deleted"
...
class AuditLog(UUIDAuditBase):
    ...
    __table_args__ = {  # noqa: RUF012
        "comment": "Tracks administrative actions for auditing and compliance"
    }
