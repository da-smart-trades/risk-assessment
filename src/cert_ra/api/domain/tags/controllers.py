...
from cert_ra.api.domain.accounts.guards import requires_active_user, requires_superuser
from cert_ra.api.domain.tags.schemas import Tag, TagCreate, TagUpdate
from cert_ra.api.domain.tags.services import TagService
from cert_ra.db.models import Tag as TagModel
...
class TagController(Controller):
    ...
    async def list_tags(
        ...
        return tags_service.to_schema(
            schema_type=Tag, data=results, total=total, filters=filters
        )
    ...
    async def get_tag(
        ...
        tags_service: TagService,
        tag_id: Annotated[
            UUID, Parameter(title="Tag ID", description="The tag to retrieve.")
        ],
    ) -> Tag:
        """Get a tag.
    ...
    async def update_tag(
        ...
        data: TagUpdate,
        tag_id: Annotated[
            UUID, Parameter(title="Tag ID", description="The tag to update.")
        ],
    ) -> Tag:
        """Update a tag.
    ...
    async def delete_tag(
        ...
        tags_service: TagService,
        tag_id: Annotated[
            UUID, Parameter(title="Tag ID", description="The tag to delete.")
        ],
    ) -> None:
        """Delete a tag."""
