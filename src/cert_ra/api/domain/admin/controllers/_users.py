...
from cert_ra.db.models import AuditAction, TeamMember, User as UserModel, UserRole
...
class AdminUserController(Controller):
    ...
    dependencies = create_service_dependencies(
        ...
        key="users_service",
        load=[
            selectinload(UserModel.roles).options(
    ...
    async def create_user_page(
        ...
        return AdminUserCreatePage(
            available_roles=[
                RoleOption(id=r.id, slug=r.slug, name=r.name) for r in all_roles
            ],
        )
    ...
    async def create_user(
        ...
        return InertiaRedirect(
            request, request.url_for("admin.users.detail", user_id=db_obj.id)
        )
    ...
    async def get_user(
        ...
        roles_service: RoleService,
        user_id: Annotated[
            UUID, Parameter(title="User ID", description="The user ID.")
        ],
    ) -> AdminUserDetailPage:
    ...
    async def unverify_user(
        ...
        db_obj = await users_service.update(
            item_id=user_id, data={"is_verified": False}
        )
        await audit_service.log_action(
            actor=current_user,
    ...
    async def assign_role(
        ...
        flash(
            request, f"Assigned role '{role.name}' to {user.email}", category="success"
        )
        return InertiaRedirect(
            request, request.url_for("admin.users.detail", user_id=user_id)
        )
    ...
    async def revoke_role(
        ...
        user_role_id: Annotated[
            UUID,
            Parameter(title="User Role ID", description="The user-role assignment ID."),
        ],
    ) -> InertiaRedirect:
        ...
        flash(
            request,
            f"Revoked role '{user_role.role_name}' from {user.email}",
            category="warning",
        )
        return InertiaRedirect(
            request, request.url_for("admin.users.detail", user_id=user_id)
