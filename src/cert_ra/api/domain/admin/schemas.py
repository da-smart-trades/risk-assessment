...
class AdminTeamDetail(CamelizedBaseStruct):
    ...
    """True for the operator team — gates the operator-role promote action."""
...
class AdminTeamMemberInvite(CamelizedBaseStruct, omit_defaults=True):
    """Invite a new member to a team via the OIDC SSO admin-provisioning flow.

    Admin-driven provisioning with allowed_email_domains soft enforcement.
    """

    email: str
    """Invitee's email address. Validated against
    ``team.allowed_email_domains`` — out-of-domain submissions require the
    ``out_of_domain_override`` flag and produce 409 Conflict otherwise."""

    role: TeamRoles = TeamRoles.MEMBER
    """Role the invitee will hold once they activate."""

    force_provider: str | None = None
    """If set, the invite link routes directly into the named provider's
    OIDC sign-in (``google`` | ``microsoft`` | ``github``). The invitee
    never sees a password-set form. NULL means the invitee chooses."""

    out_of_domain_override: bool = False
    """Set to True to acknowledge the email is outside the team's allowed
    domains. The new row is flagged for audit and the request proceeds.
    Without it, an out-of-domain email returns 409."""
