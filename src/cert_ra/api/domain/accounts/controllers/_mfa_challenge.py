...
from cert_ra.api.domain.accounts.schemas import MfaChallenge
...
class MfaChallengeController(Controller):
    ...
    signature_namespace = {"UserService": UserService, "MfaChallenge": MfaChallenge}  # noqa: RUF012
    cache = False
    ...
    async def verify_challenge(
        ...
        if result.mfa_disabled:
            flash(
                request, "Your account was successfully authenticated.", category="info"
            )
        elif result.used_backup_code:
            if result.remaining_backup_codes <= 2:  # noqa: PLR2004
                flash(
                    request,
                ...
                flash(
                    request,
                    "Successfully authenticated with recovery code.",
                    category="info",
                )
        else:
            flash(
