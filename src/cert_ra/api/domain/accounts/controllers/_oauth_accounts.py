...
from cert_ra.api.domain.accounts.dependencies import (
...
from cert_ra.api.domain.accounts.schemas import User
from cert_ra.api.domain.accounts.services import UserOAuthAccountService, UserService
from cert_ra.api.lib.oauth import (
    AccessTokenState,
    OAuth2AuthorizeCallback,
    get_oauth2_github_client,
    get_oauth2_google_client,
)
...
OAUTH_CLIENTS: dict[str, BaseOAuth2] = {
    "github": get_oauth2_github_client(),
    "google": get_oauth2_google_client(),
}
...
class OAuthAccountController(Controller):
    ...
    async def unlink_oauth(
        ...
        can_unlink, reason = await oauth_account_service.can_unlink_oauth(
        ...
        flash(
            request,
            f"Successfully disconnected {provider.title()} account.",
            category="success",
        )
        return InertiaRedirect(request, request.url_for("profile.show"))
    ...
    async def complete_link_oauth(
        ...
        current_user: UserModel,
        oauth_account_service: UserOAuthAccountService,
        users_service: UserService,  # noqa: ARG002
    ) -> InertiaRedirect:
        ...
        state_key = next(
            (
                key
                for key in (
                    f"oauth_state:link:{provider}",
                    f"oauth_state:upgrade:{provider}",
                )
        ...
        callback = OAuth2AuthorizeCallback(
            client, route_name="oauth.link.complete", state_session_key=state_key
        )
        try:
            access_token_state: AccessTokenState = await callback(request)
        except Exception as e:  # noqa: BLE001
            request.logger.warning(
                "OAuth callback failed", error=str(e), provider=provider
            )
            flash(
                request,
                f"Failed to connect {provider.title()} account. Please try again.",
                category="error",
            )
            return InertiaRedirect(request, request.url_for("profile.show"))
        ...
        try:
            account_id, account_email = await client.get_id_email(
                token=token["access_token"]
            )
        except Exception as e:  # noqa: BLE001
            request.logger.warning(
                "Failed to get OAuth user info", error=str(e), provider=provider
            )
            flash(
                request,
                f"Failed to get account info from {provider.title()}.",
                category="error",
            )
            return InertiaRedirect(request, request.url_for("profile.show"))
        ...
        existing = await oauth_account_service.get_by_provider_account_id(
            provider, account_id
        )
        if existing and existing.user_id != current_user.id:
            flash(
                request,
                f"This {provider.title()} account is already linked to another user.",
                category="error",
            )
            return InertiaRedirect(request, request.url_for("profile.show"))
        ...
        scopes = (
            ...
            if token.get("scope")
            else OAUTH_DEFAULT_SCOPES.get(provider)
        )
        ...
        flash(
            request,
            f"Successfully connected {provider.title()} account.",
            category="success",
        )
        return InertiaRedirect(request, request.url_for("profile.show"))
