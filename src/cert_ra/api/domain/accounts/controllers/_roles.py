...
from cert_ra.api.domain.accounts.schemas import UserRoleAdd, UserRoleRevoke
...
class RoleController(Controller):
    ...
    tags = ["Roles"]  # noqa: RUF012
    guards = [requires_superuser]  # noqa: RUF012
    dependencies = {"roles_service": Provide(provide_roles_service)}  # noqa: RUF012


class UserRoleController(Controller):
    """Handles the adding and removing of User Role records."""

    tags = ["User Account Roles"]  # noqa: RUF012
    guards = [requires_superuser]  # noqa: RUF012
    ...
    async def assign_role(
        ...
        obj, created = await user_roles_service.get_or_upsert(
            role_id=role_id, user_id=user_obj.id, upsert=False
        )
        if created:
            return Message(
                message=f"Successfully assigned the '{obj.role_slug}' role to {obj.user_email}."
            )
        return Message(
            message=f"User {obj.user_email} already has the '{obj.role_slug}' role."
        )
    ...
    async def revoke_role(
        ...
        role_slug: str = Parameter(
            title="Role Slug", description="The role to revoke."
        ),
    ) -> Message:
        ...
        return Message(
            message=f"Removed the '{role_slug}' role from User {user_obj.email}."
        )
