...
from cert_ra.api.domain.accounts.schemas import User, UserCreate, UserUpdate
...
class UserController(Controller):
    ...
    async def list_users(
        ...
        return users_service.to_schema(
            data=results, total=total, schema_type=User, filters=filters
        )
    ...
    @patch(
        operation_id="UpdateUser", name="users:update", path="/api/users/{user_id:uuid}"
    )
    async def update_user(
        self,
        data: UserUpdate,
        users_service: UserService,
        user_id: UUID = Parameter(title="User ID", description="The user to update."),  #  noqa: B008
    ) -> User:
