...
from cert_ra.api.domain.accounts.signals import UserInfo
from cert_ra.api.domain.web.email import EmailMessageService
from cert_ra.api.lib.schema import VerifyEmailPage
...
class EmailVerificationController(Controller):
    ...
    async def verify_email(
        ...
        if user.is_verified:
            ...
            await users_service.update(
                {"is_verified": True}, item_id=user.id, auto_commit=True
            )
    ...
    async def resend_verification_email(
        ...
        await email_token_service.invalidate_existing_tokens(
            email=user.email, token_type=TokenType.EMAIL_VERIFICATION
        )
        expires_delta = timedelta(
            hours=get_email_settings().verification_token_expires_hours
        )
        ...
        return InertiaRedirect(
            request, request.url_for("verify-email", status="verification-link-sent")
        )
