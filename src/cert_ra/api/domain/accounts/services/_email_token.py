...
from cert_ra.db.models import EmailToken, TokenType
...
class EmailTokenService(SQLAlchemyAsyncRepositoryService[EmailToken]):
    ...
    async def to_model_on_create(
        self, data: ModelDictT[EmailToken]
    ) -> ModelDictT[EmailToken]:
    ...
    async def validate_token(
        ...
        """
        token = await self.get_one_or_none(
            token_hash=self._hash_token(plain_token), token_type=token_type
        )
    ...
    async def invalidate_existing_tokens(
        self, email: str, token_type: TokenType
    ) -> int:
        ...
        tokens = [
            t
            for t in await self.list(email=email, token_type=token_type, used_at=None)
            if t.is_valid
        ]
        for token in tokens:
            token.mark_used()
