...
from cert_ra.db.models import TeamMember, User, UserRole
...
class MfaVerifyResult:
    ...
    def __init__(
        ...
        mfa_disabled: bool = False,  # noqa: FBT001, FBT002
        used_backup_code: bool = False,  # noqa: FBT001, FBT002
...
async def generate_backup_codes(count: int = 8) -> tuple[list[str], list[str]]:
    ...
    hashed_codes: list[str] = [
        await crypt.hash_backup_code(code) for code in plain_codes
    ]
    return plain_codes, hashed_codes


def generate_qr_code(
    secret: str, email: str, issuer: str = "Certora Risk Assessment"
) -> str:
...
class UserService(SQLAlchemyAsyncRepositoryService[User]):
    ...
    async def authenticate(self, username: str, password: bytes | str) -> User:
        ...
        db_obj = await self.get_one_or_none(
            email=username, load=[undefer_group("security_sensitive")]
        )
        if db_obj is None:
    ...
    async def update_password(self, data: dict[str, Any], db_obj: User) -> None:
        ...
        user_with_creds = await self.get_one_or_none(
            id=db_obj.id, load=[undefer_group("security_sensitive")]
        )
        ...
        if not await crypt.verify_password(
            data["current_password"], user_with_creds.hashed_password
        ):
            msg = "User not found or password invalid."
            raise PermissionDeniedException(detail=msg)
        if not user_with_creds.is_active:
            ...
            raise PermissionDeniedException(detail=msg)
        user_with_creds.hashed_password = await crypt.get_password_hash(
            data["new_password"]
        )
    ...
    async def verify_mfa(
        ...
        user = await self.get_one_or_none(
            email=email, load=[undefer_group("security_sensitive")]
        )
        if not user:
            msg = "User not found"
            raise PermissionDeniedException(detail=msg)
        ...
        if recovery_code and user.backup_codes:
            code_index = await crypt.verify_backup_code(
                recovery_code, user.backup_codes
            )
            if code_index is not None:
                ...
                await self.update(
                    item_id=user.id, data={"backup_codes": updated_codes or None}
                )
    ...
    async def has_role(db_obj: User, role_name: str) -> bool:
        ...
        """
        return any(
            assigned_role.role_name == role_name for assigned_role in db_obj.roles
        )
    ...
    async def delete_if_orphaned(self, user_id: UUID) -> bool:
        """Delete a user iff they have no remaining team memberships.

        Called after a ``TeamMember`` row is removed so that a user whose
        last team membership just disappeared is purged from the database
        rather than left as an unreachable account.

        Skipped (returns ``False``) when:

        * the user no longer exists,
        * the user is a superuser (admins can exist without a team), or
        * a ``RESTRICT`` foreign key blocks deletion (e.g. the user
          authored a manual_metric / weighting_profile / alert). The
          attempt runs in a savepoint so the surrounding team-removal
          transaction is not poisoned by the rollback.
        ...
        try:
            async with session.begin_nested():
                _ = await self.delete(user_id)
        except IntegrityError as exc:
            await logger.awarning(
                "Skipped auto-delete of orphaned user; FK references remain.",
                user_id=str(user_id),
                error=str(exc),
            )
            return False
    ...
    ALLOWED_AVATAR_TYPES: set[str] = {  # noqa: RUF012
        "image/jpeg",
        "image/png",
        "image/gif",
        "image/webp",
    ...
    async def upload_avatar(
        self, user: User, content: bytes, content_type: str, original_filename: str
    ) -> User:
        ...
        ext = Path(original_filename).suffix.lower() or self._get_extension(
            content_type
        )
        ...
        # plugin (1.9.x) does not register the FileObject save listener, so the
        # save-on-commit behaviour never fires and the avatar would be lost.
        await avatar.save_async()
        user.avatar = avatar
