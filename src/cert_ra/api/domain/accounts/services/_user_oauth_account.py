...
from cert_ra.db.models import User, UserOauthAccount
...
class UserOAuthAccountService(SQLAlchemyAsyncRepositoryService[UserOauthAccount]):
    ...
    async def can_unlink_oauth(self, user: User, provider: str) -> tuple[bool, str]:  # noqa: ARG002
        ...
        """
        # Query hashed_password explicitly rather than reading
        # `user.hashed_password` — the attribute is deferred on the model,
        # and the `current_user` passed in by web controllers is loaded
        # via provide_users_service, which doesn't include
        # undefer_group("security_sensitive"). Touching the attribute
        # would trigger an implicit deferred-column load that raises
        # MissingGreenlet under AsyncSession.
        has_password = await self.repository.session.scalar(
