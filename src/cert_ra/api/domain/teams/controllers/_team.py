...
from cert_ra.api.domain.teams.services import TeamInvitationService, TeamService
...
from cert_ra.db.models.team_member import TeamMember as TeamMemberModel
...
class TeamController(Controller):
    ...
    async def list_teams(
        ...
        for team in results:
            membership = next(
                (m for m in team.members if m.user_id == current_user.id), None
            )
            user_role = (
                "owner"
    ...
    async def create_team(
        ...
        return InertiaRedirect(
            request, request.url_for("teams.show", team_slug=db_obj.slug)
        )
    ...
    async def get_team(
        ...
        invitations = (
            await team_invitations_service.get_pending_for_team(db_obj.id)
            if is_admin
            else []
        )
        ...
        return TeamDetailPage(
            team=TeamDetail(
                ...
                tags=[
                    TeamTag(id=tag.id, slug=tag.slug, name=tag.name)
    ...
    async def get_team_settings(
        ...
        return TeamDetailPage(
            team=TeamDetail(
                ...
                description=db_obj.description,
                domain=db_obj.domain,
                enforced_provider=db_obj.enforced_provider,
                tags=[
                    TeamTag(id=tag.id, slug=tag.slug, name=tag.name)
                    for tag in db_obj.tags
                ],
            ...
            enforcement_enabled=get_feature_settings().enforced_provider,
        )
    ...
    async def delete_team(
        ...
        db_obj = await teams_service.get_one(slug=team_slug)
        # Snapshot member ids before cascade-delete; skip the actor so the
        # owner deleting their own workspace doesn't silently delete their
        # own account in the process.
        candidate_user_ids = [
            m.user_id for m in db_obj.members if m.user_id != current_user.id
        ]
        db_obj = await teams_service.delete(db_obj.id)
        for user_id in candidate_user_ids:
            await users_service.delete_if_orphaned(user_id)
