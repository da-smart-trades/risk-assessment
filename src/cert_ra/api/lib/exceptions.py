...
from litestar_vite.inertia.exception_handler import (
    create_inertia_exception_response as inertia_exception_handler,
)
...
class ApplicationError(Exception):
    ...
    def __init__(self, *args: Any, detail: str = "") -> None:  # noqa: ANN401
...
def integrity_error_handler(
    request: Request[UserT, AuthT, StateT], exc: IntegrityError
) -> Response[Any]:
    ...
    return cast("Response[Any]", InertiaBack(request))


def not_found_error_handler(
    request: Request[UserT, AuthT, StateT], exc: NotFoundError
) -> Response[Any]:
    ...
    """
    detail = (
        exc.detail if hasattr(exc, "detail") and exc.detail else "Resource not found"
    )
...
def repository_error_handler(
    request: Request[UserT, AuthT, StateT], exc: RepositoryError
) -> Response[Any]:
...
def get_exception_handlers() -> ExceptionHandlersMap:
    """Get the lib's exception handlers.

    Returns:
        A mapping of exception types to handler functions.
    """
    return {
